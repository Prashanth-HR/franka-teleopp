...
#include <tele_operation/teleop_joint_pd_example_controller.h>
...
bool TeleopJointPDExampleController::init(hardware_interface::RobotHW* robot_hw,
  ...
  if (debug_) {
    ...
    follower_jacob_pub_.init(node_handle, "follower_jacobian", 1);
...
void TeleopJointPDExampleController::initArm(hardware_interface::RobotHW* robot_hw,
  ...
  arm_data.virtual_joint_wall = std::make_unique<franka_example_controllers::JointWallContainer<7>>(
...
void TeleopJointPDExampleController::update(const ros::Time& /*time*/,
  ...
  std::array<double, 42> endeffector_zero_jacobian =
        follower_data_.model_handle->getZeroJacobian(franka::Frame::kEndEffector);
        
  if (follower_jacob_pub_.trylock()) {
    follower_jacob_pub_.msg_.data.resize(42)
    follower_jacob_pub_.msg_.data = endeffector_zero_jacobian;
    follower_jacob_pub_.unlockAndPublish();
  }
...
PLUGINLIB_EXPORT_CLASS(tele_operation::TeleopJointPDExampleController,
